 ...
 */
const wordList = [
  "muff", "keks", "blub", "pups", "koks", "mampfi", "zapp", "zisch",
  "peng", "knall", "bumm", "huhu", "lala", "klo", "ups", "tada",
  "lol", "haha", "bibber", "mops", "dope", "kack", "grins", "zack",
  "muh", "wuff", "miau", "wurm", "igitt", "knick", "knack", "fluff",
  "nagel", "kokos", "wusel", "umami", "flausch", "hopps", "klacks", "boing",
  "schmatz", "po", "knuff", "glubsch", "ei", "kusche", "dada", "kroko",
  "igel", "wonne", "plopp", "auf", "uff", "zubb", "dalli", "rappel",
  "kicher", "minz", "knirsch", "bumms", "mampf", "blubb", "quark", "dudu",
  "flotti", "knuffig", "kauder", "schnuff", "tata", "pille", "wimmel",
  "tüdel", "gacker", "ringel", "zappel", "zucki", "kringel", "blitz", "sause",
  "wimpel", "gay", "witzig", "nuckel", "hops", "geil", "toll", "quirlig"
];
...
io.on('connection', (socket) => {
  ...
  socket.on('createLobby', ({ name }) => {
    if (!name || !name.trim()) {
      socket.emit('errorMessage', 'Name ist erforderlich, um eine Lobby zu erstellen.');
      console.log(`Lobby-Erstellung fehlgeschlagen: Kein Name angegeben von ${socket.id}`);
      return;
    }
  ...
  socket.on('joinLobby', ({ code, name }) => {
    ...
    if (!lobby) {
      ...
      console.log(`Lobby-Beitritt fehlgeschlagen: Lobby ${code} existiert nicht. Versucht von ${socket.id}`);
      return;
    }
    if (lobby.started) {
      socket.emit('errorMessage', 'Das Spiel in dieser Lobby läuft bereits.');
      console.log(`Lobby-Beitritt fehlgeschlagen: Spiel in Lobby ${code} läuft bereits. Versucht von ${socket.id}`);
      return;
    }
    if (!name || !name.trim()) {
      socket.emit('errorMessage', 'Name ist erforderlich, um einer Lobby beizutreten.');
      console.log(`Lobby-Beitritt fehlgeschlagen: Kein Name angegeben von ${socket.id}`);
      return;
    }
    ...
    console.log(`Spieler ${name} (${socket.id}) ist der Lobby ${code} beigetreten.`);
  ...
  socket.on('beginRoleAssignment', (code) => {
    ...
    if (!lobby) {
      socket.emit('errorMessage', 'Lobby existiert nicht.');
      console.log(`Rollenvergabe fehlgeschlagen: Lobby ${code} existiert nicht. Versucht von ${socket.id}`);
      return;
    }

    if (socket.id !== lobby.hostId) {
      socket.emit('errorMessage', 'Nur der Host kann die Rollenvergabe starten.');
      console.log(`Rollenvergabe fehlgeschlagen: ${socket.id} ist nicht der Host von Lobby ${code}`);
      return;
    }

    if (lobby.started) {
      socket.emit('errorMessage', 'Das Spiel hat bereits begonnen.');
      console.log(`Rollenvergabe fehlgeschlagen: Spiel in Lobby ${code} hat bereits begonnen.`);
      return;
    }

    if (lobby.players.length < 2) {
      socket.emit('errorMessage', 'Mindestens 2 Spieler sind erforderlich, um die Rollenvergabe zu starten.');
      console.log(`Rollenvergabe fehlgeschlagen: Weniger als 2 Spieler in Lobby ${code}.`);
      return;
    }

    console.log(`Host (${socket.id}) startet die Rollenvergabe in Lobby ${code}.`);
    ...
    players.forEach((p) => {
      ...
      console.log(`Spieler ${p.id} soll eine Rolle für ${targetPlayer.id} vergeben.`);
    });
  });
  ...
  socket.on('submitRole', ({ lobbyCode, playerId, role }) => {
    ...
    if (!lobby) {
      socket.emit('errorMessage', 'Lobby existiert nicht.');
      console.log(`Rolle vergeben fehlgeschlagen: Lobby ${lobbyCode} existiert nicht. Versucht von ${socket.id}`);
      return;
    }

    if (lobby.started) {
      socket.emit('errorMessage', 'Das Spiel hat bereits begonnen.');
      console.log(`Rolle vergeben fehlgeschlagen: Spiel in Lobby ${lobbyCode} hat bereits begonnen.`);
      return;
    }
    ...
    if (!playerExists) {
      ...
      console.log(`Rolle vergeben fehlgeschlagen: ${socket.id} ist kein Teil der Lobby ${lobbyCode}.`);
      return;
    }
    ...
    if (Object.keys(lobby.roles).length === lobby.players.length) {
      ...
      console.log(`Spieler ${socket.id} hat eine Rolle vergeben. Warte auf weitere Rollen.`);
    }
  });
  ...
  socket.on('playerGuessed', ({ code, playerId }) => {
    ...
    if (!lobby) {
      socket.emit('errorMessage', 'Lobby existiert nicht.');
      console.log(`Spielererratung fehlgeschlagen: Lobby ${code} existiert nicht. Versucht von ${socket.id}`);
      return;
    }

    if (socket.id !== lobby.hostId) {
      socket.emit('errorMessage', 'Nur der Host kann Spieler als erraten markieren.');
      console.log(`Spielererratung fehlgeschlagen: ${socket.id} ist nicht der Host von Lobby ${code}`);
      return;
    }
    ...
    if (!lobby.guessed[playerId]) {
      ...
      lobby.guessCount += 1;
      console.log(`Spieler ${playerId} wurde als erraten markiert in Lobby ${code}.`);
    } else {
      console.log(`Spieler ${playerId} wurde bereits als erraten markiert in Lobby ${code}.`);
